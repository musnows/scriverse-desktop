...
function emptyProviderConfiguration() {
  return '<div class="empty-state"><b>尚未配置 AI 供应商</b>添加后端返回的供应商协议、接口地址和凭据，测试成功后再添加模型。</div>';
}
...
export function renderAiProviderConfigurationCards(providers, models, protocolOptions = []) {
  ...
  return `<div class="card-grid provider-card-grid">${providers.map((provider) => {
    ...
    const disabledNotice = provider.status === "disabled"
      ...
      : "";
    return `
    <article class="record-card provider-card ${provider.status === "disabled" ? "is-disabled" : ""}"><div class="provider-card-meta"><small><span class="provider-scope-badge ${provider.scope === "local" ? "is-local" : ""}">${esc(providerScopeLabel(provider))}</span> · ${esc(providerProtocolLabel(provider.protocol, protocolOptions))} · ${esc(providerConnectionLabel(provider.connectionStatus))}</small><span class="provider-status-badge ${providerStatusClass}">${esc(providerStatusLabel(provider.status))}</span></div><h3>${esc(provider.name)}</h3>
    ${disabledNotice}<p>${esc(provider.baseUrl)}\n密钥：${esc(provider.apiKey)}\n最大输出参数：${esc(provider.maxTokensParameter ?? "max_tokens")}\n思考类型：${esc(provider.thinkingType ?? "enabled")}\n并发：${Number(provider.concurrencyLimit) || 10} · 每分钟请求：${Number(provider.rpmLimit) || 10}${provider.scope === "local" ? "" : `\n每日 Token 额度：${provider.dailyTokenQuota === null || provider.dailyTokenQuota === undefined ? "未限制" : Number(provider.dailyTokenQuota).toLocaleString("zh-CN")} · 每月 Token 额度：${provider.monthlyTokenQuota === null || provider.monthlyTokenQuota === undefined ? "未限制" : Number(provider.monthlyTokenQuota).toLocaleString("zh-CN")}`}${provider.lastError ? `\n错误：${esc(provider.lastError)}` : ""}</p>
