...
export class LocalAiRequestCoordinator {
  ...
  async complete(value: LocalAiCompletionRequestInput, onEvent?: (event: LocalAiStreamEvent) => void): Promise<LocalAiCompletionResult> {
    const input = parseLocalAiCompletionRequestInput(value);
    if (this.active.has(input.requestId)) {
      throw new LocalAiRequestCoordinatorError("LOCAL_AI_REQUEST_IN_PROGRESS", "相同 AI 请求正在处理中");
    }
    ...
    try {
      ...
      return await this.client.complete(credential, completion, controller.signal, onEvent);
    } finally {
      this.active.delete(input.requestId);
    }
  }

  async completeAgentRound(value: LocalAiAgentRoundInput, onEvent?: (event: LocalAiStreamEvent) => void): Promise<LocalAiAgentRoundResult> {
    const input = parseLocalAiAgentRoundInput(value);
    if (this.active.has(input.requestId)) {
      throw new LocalAiRequestCoordinatorError("LOCAL_AI_REQUEST_IN_PROGRESS", "相同 AI Agent 轮次正在处理中");
    }
    const controller = new AbortController();
    this.active.set(input.requestId, controller);
    try {
      return await this.client.completeAgentRound(this.store.credential(input.modelId), input, controller.signal, onEvent);
    } finally {
      this.active.delete(input.requestId);
    }
  }
