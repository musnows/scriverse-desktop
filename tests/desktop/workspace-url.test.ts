...
describe("Desktop 本地工作区导航", () => {
  it("只接受随机端口上的 IPv4 回环 origin", () => {
    expect(normalizeLocalWorkspaceOrigin("http://127.0.0.1:14321")).toBe("http://127.0.0.1:14321");
    expect(() => normalizeLocalWorkspaceOrigin("http://localhost:14321")).toThrowError(/origin/u);
    expect(() => normalizeLocalWorkspaceOrigin("https://127.0.0.1:14321")).toThrowError(/origin/u);
    expect(() => normalizeLocalWorkspaceOrigin("http://127.0.0.1:14321/path")).toThrowError(/origin/u);
  });

  it("允许同源页面路径并阻止跨源、凭据和非法 URL", () => {
    const origin = "http://127.0.0.1:14321";
    ...
    expect(isAllowedWorkspaceNavigation("http://user:pass@127.0.0.1:14321/", origin)).toBe(false);
